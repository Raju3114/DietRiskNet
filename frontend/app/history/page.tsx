...
interface HistoryMeal {
  ...
  created_at: string;
  items?: FoodItem[];
  // Nullable: null means the meal had insufficient data (e.g. DCI needs
  // >=2 longitudinal days) — it must never be rendered as 0 / perfect health.
    ...
    return (
      <ProtectedRoute>
        <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
          ...
          <span className="text-muted-foreground text-xs font-semibold uppercase tracking-widest animate-pulse">Compiling meal history logs...</span>
        </div>
      </ProtectedRoute>
    );
  }
    ...
    return (
      <ProtectedRoute>
        <div className="p-8 rounded-2xl bg-charcoal-medium border border-brand-red/20 text-center max-w-md mx-auto my-12 shadow-lg">
          ...
          <p className="text-muted-foreground text-xs mt-1">Please check if the backend API is up and running.</p>
        </div>
      </ProtectedRoute>
    );
  }
    ...
    <ProtectedRoute>
      <div className="space-y-8 animate-fade-in font-sans">
        ...
        <div className="border-b border-charcoal-border/50 pb-6">
          ...
          <p className="text-muted-foreground text-[10px] font-extrabold uppercase tracking-wider mt-1">Review all registered meals and visual bounding box detections over time.</p>
        </div>
        ...
        {history.length === 0 ? (
          ...
          >
            ...
            <div>
              <h3 className="text-xs font-bold text-foreground mb-1 uppercase tracking-wider">No Meals Logged Yet</h3>
              <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest leading-relaxed">Upload a photo of your meal to start tracking calories, macronutrients, and risks.</p>
            </div>
            <button 
              onClick={() => router.push('/upload')}
              className="px-6 py-3 bg-brand-primary hover:bg-brand-primary-hover hover:shadow-brand-primary/10 border border-brand-primary/20 text-white text-[9px] font-bold uppercase tracking-widest rounded-xl shadow-lg transition-all duration-200 cursor-pointer"
            >
          ...
          >
            {history.map((meal: HistoryMeal) => {
              // Null risk = insufficient data; never fabricate a "low" risk.
              const hasRisk = meal.risk_score != null && meal.risk_level != null;
              const hasDci = meal.dci != null;
              const hasNis = meal.nis != null;
              const riskLevel = (meal.risk_level || '').toLowerCase();
              let riskBadgeClass = 'bg-charcoal-light/60 text-muted-foreground border-charcoal-border';
              ...
              if (riskLevel === 'high') {
                ...
                riskBadgeClass = 'bg-brand-emerald/10 text-brand-emerald border-brand-emerald/20';
                riskTextColor = 'text-brand-emerald';
              }
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-charcoal-border/50 pb-4">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className={`px-3 py-1.5 rounded-xl border text-[9px] font-extrabold uppercase tracking-widest ${riskBadgeClass}`}>
                        {hasRisk ? (meal.risk_level || '').toUpperCase() : 'INSUFFICIENT DATA'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-2">
                      <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Recognized Foods</span>
                      <div className="flex flex-wrap gap-1.5">
                        {meal.items && meal.items.length > 0 ? (
                          ...
                          <span className="text-[9px] text-muted-foreground uppercase tracking-wider font-bold">No crops identified</span>
                        )}
                      </div>
                    </div>
                    ...
                    {/* Nutrients summary */}
                    <div className="space-y-2">
                      <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Nutritional Breakdown</span>
                      <div className="grid grid-cols-2 gap-2 text-[9px] font-bold text-muted-foreground bg-charcoal-light/40 p-3.5 rounded-xl border border-charcoal-border/50">
                        <div className="flex justify-between border-b border-charcoal-border/30 pb-1">
                          <span className="text-muted-foreground uppercase tracking-wider">Calories:</span>
                          <span className="text-brand-orange font-bold font-mono">{(meal.calories || 0).toFixed(0)} kcal</span>
                        </div>
                        <div className="flex justify-between border-b border-charcoal-border/30 pb-1">
                          <span className="text-muted-foreground uppercase tracking-wider">Carbs:</span>
                        ...
                        <div className="flex justify-between pt-1">
                          <span className="text-muted-foreground uppercase tracking-wider">Fats:</span>
                    ...
                    {/* Indices */}
                    <div className="space-y-2">
                      <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">Risk &amp; Consistency Indices</span>
                      <div className="grid grid-cols-2 gap-2 text-[9px] font-bold text-muted-foreground bg-charcoal-light/40 p-3.5 rounded-xl border border-charcoal-border/50">
                        ...
                        <div className="flex flex-col justify-center pl-2">
                          <span className="text-muted-foreground uppercase tracking-wider block text-[8px]">Imbalance (NIS)</span>
                          <span className="text-brand-orange text-base font-black mt-1 font-mono">
                            {hasNis ? meal.nis!.toFixed(2) : 'N/A'}
                          </span>
                        </div>
                      </div>
                    </div>
